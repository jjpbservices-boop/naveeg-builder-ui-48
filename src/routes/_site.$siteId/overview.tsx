...
export default function OverviewRoute() {
  ...
  const { data: domains } = useQuery({
    ...
    select: (r) => {
      const def = r.data.find(d => d.default === 1);
      return { defaultDomain: def?.name, hasCustom: r.data.some(d => !d.name.endsWith(".10web.club")) };
    },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <div className="p-4 rounded-2xl border">
          ...
          <Link to="/_site/$siteId/domains" params={{ siteId: siteId }} className="text-sm underline">{t("manage_domains", { ns: "common" })}</Link>
        </div>

        <div className="p-4 rounded-2xl border">
          <div className="font-medium mb-2">{t("security")}</div>
          <p className="text-sm mb-3">Staging: {settings?.data.staging_enabled ? t("enabled", { ns: "common" }) : t("disabled", { ns: "common" })}</p>
          <Link to="/_site/$siteId/security" params={{ siteId: siteId }} className="text-sm underline">{t("backups_restore", { ns: "common" })}</Link>
        </div>

        <div className="p-4 rounded-2xl border">
          <div className="font-medium mb-2">{t("billing")}</div>
          <p className="text-sm mb-3">{t("plan_info_here")}</p>
          <Link to="/_site/$siteId/billing" params={{ siteId: siteId }} className="text-sm underline">{t("open_billing", { ns: "common" })}</Link>
        </div>
      </div>
